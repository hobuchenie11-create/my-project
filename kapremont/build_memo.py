...
wb = Workbook()
wb.remove(wb.active)   # пустой лист «Sheet» из шаблона openpyxl
...
подсказка = lst.cell(ПОСЛЕДНЯЯ + 2, 2,
                     ...
                     "переносить ничего не нужно. На этом листе формул нет: "
                     "всё, что считает памятка, вынесено на скрытый лист "
                     "«СЛУЖЕБНЫЙ», поэтому сортировка и протяжка здесь "
                     "ничего не сломают. Лишние строки просто очистите.")
...
# ==========================================================================
# Скрытый лист СЛУЖЕБНЫЙ — нумерация квартир внутри каждого статуса
# ==========================================================================
# Раньше эти формулы стояли прямо в списке квартир, и при сортировке или
# протяжке ячеек их затирало: часть превращалась в числа, часть пропадала,
# и памятка начинала показывать пропуски. Теперь они живут отдельно и
# ссылаются на список, а трогать их не нужно.
SRV = "СЛУЖЕБНЫЙ"
srv = wb.create_sheet(SRV)
for i, (код, _, _) in enumerate(СТАТУСЫ):
    c = srv.cell(1, 1 + i, f"№ в «{код}»")
    c.font = Font(name=FONT, size=9, bold=True)
for n in range(ВСЕГО_КВАРТИР):
    строка_списка = ПЕРВАЯ_СТРОКА_СПИСКА + n
    for i, (код, _, _) in enumerate(СТАТУСЫ):
        srv.cell(n + 2, 1 + i).value = (
            f'=IF(\'{LIST}\'!$C{строка_списка}="{код}",'
            f'COUNTIF(\'{LIST}\'!$C${ПЕРВАЯ_СТРОКА_СПИСКА}:'
            f'$C{строка_списка},"{код}"),"")')
srv.sheet_state = "hidden"

СЛУЖ_ПЕРВАЯ, СЛУЖ_ПОСЛЕДНЯЯ = 2, ВСЕГО_КВАРТИР + 1
...
for i, (код, заголовок, пояснение) in enumerate(СТАТУСЫ):
    лев = chr(ord("B") + i * 2)
    прав = chr(ord("C") + i * 2)
    служебный = chr(ord("A") + i)   # столбец на листе СЛУЖЕБНЫЙ
    ...
    for k in range(СТРОК_В_ПОДКОЛОНКЕ):
        ...
        for половина, колонка in enumerate((лев, прав)):
            ...
            c.value = (
                f'=IFERROR("кв. "&INDEX(\'{LIST}\'!'
                f'$B${ПЕРВАЯ_СТРОКА_СПИСКА}:$B${ПОСЛЕДНЯЯ},'
                f"MATCH({позиция},'{SRV}'!"
                f'${служебный}${СЛУЖ_ПЕРВАЯ}:${служебный}${СЛУЖ_ПОСЛЕДНЯЯ},0)),"")')
