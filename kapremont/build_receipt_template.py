...
CFG_ROWS = [
    ...
    ("День срока оплаты", 15,
      "Единый срок по ч. 1 ст. 155 и ч. 2 ст. 171 ЖК РФ (ФЗ от 24.06.2025 "
      "№ 177-ФЗ, действует с 01.03.2026). Решением общего собрания не меняется",
      None),
...
put(ws, "B42:H42",
    f'="Начисление за "&{PERIOD_TXT}&".    По вопросам начислений '
    f'обращайтесь к председателю совета дома, тел. "'
    f'&{CFG_REF["Контактный телефон"]}&"."',
...
LINES = [
    ...
    ("", ""),
    ("s", "6. Сроки"),
    ("t", "Взнос вносится до 15 числа месяца, следующего за расчётным — "
          "единый срок по ч. 1 ст. 155 и ч. 2 ст. 171 ЖК РФ в редакции "
          "Федерального закона от 24.06.2025 № 177-ФЗ (действует с 01.03.2026). "
          "Он распространяется и на взносы на капитальный ремонт по "
          "специальному счёту; решением общего собрания его изменить нельзя."),
    ("t", "Тем же законом сдвинут срок вручения платёжного документа: его "
          "нужно представить собственнику не позднее 5 числа месяца, "
          "следующего за расчётным (раньше было не позднее 1 числа). "
          "Планируйте разноску и рассылку квитанций с запасом до этой даты."),
    ("", ""),
    ("s", "7. Что проверить перед первой выдачей квитанций"),
...
NAMES = {
    ...
    "ДАННЫЕ_ТАБЛИЦА": f"{DATA}!$A$1:$O$5000",
}
