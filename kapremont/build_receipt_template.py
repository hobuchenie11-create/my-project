...
CFG_ROWS = [
    ...
    ("Срок оплаты", dt.datetime(2026, 8, 10),
     "До какого числа вносится взнос", DATEFMT),
...
DATA_COLS = [
    ...
    ("Накоплено по лицевому счёту, руб.", 26, MONEY),
    ("Расчётный период (текст)", 20, None),
    ("Дата начала периода", 18, DATEFMT),
    ("Срок оплаты", 14, DATEFMT),
    ("№ квитанции", 18, None),
]
...
DEMO = [31202, 49, "Иванов Иван Иванович",
        ...
        674.99, 19501.38, 0, 0, 20176.37, 13441.28, None, None,
        "Июль 2026", dt.datetime(2026, 7, 1), dt.datetime(2026, 8, 10),
        "КР-2026-07-31202"]
...
hint = data.cell(4, 1,
                 ...
                 "Столбцы F, K, M, N можно оставить пустыми — бланк рассчитает "
...
WIDTHS = {"A": 1.8, "B": 21.0, "C": 12.5, "D": 11.5, "E": 11.0,
          "F": 12.0, "G": 12.5, "H": 14.5, "I": 1.8}
for col, w in WIDTHS.items():
    ws.column_dimensions[col].width = w
ws.sheet_view.showGridLines = False

PERIOD_TXT = (f'IF({lookup("$O")}="",{CFG_REF["Расчётный период (текст)"]},'
              f'{d("$O")})')
PERIOD_DATE = (f'IF({lookup("$P")}="",{CFG_REF["Дата начала периода"]},'
               f'{d("$P")})')
DUE_DATE = (f'IF({lookup("$Q")}="",{CFG_REF["Срок оплаты"]},{d("$Q")})')
...
CALC_ROWS = [
    ...
    (23, "Пени за несвоевременную уплату взносов", None,
     f"={lookup('$I', empty='0')}"),
    (24, "Перерасчёты / переплата (уменьшают сумму к оплате)", None,
     f"={lookup('$J', empty='0')}"),
]
...
put(ws, "B25:G25", "ИТОГО К ОПЛАТЕ", role="total", size=13, bold=True, indent=1)
put(ws, "H25", f'=IF({lookup("$K")}="",ROUND(H21+H22+H23+H24,2),{d("$K")})',
...
REF_ROWS = [
    (28, f'="Поступило оплат по лицевому счёту с начала "&YEAR({PERIOD_DATE})'
         f'&" года, руб."',
     f"={lookup('$L', empty='0')}"),
    (29, "Размер фонда капитального ремонта, накопленного по лицевому счёту, руб.",
     f'=IF({lookup("$N")}="","—",{d("$N")})'),
    (30, "Остаток средств на специальном счёте многоквартирного дома, руб.",
     f'=IF({lookup("$M")}="","—",{d("$M")})'),
]
...
put(ws, "B32:F32", "5. ПОРЯДОК ОПЛАТЫ", role="section", size=9, bold=True,
    indent=1)
put(ws, "G32:H32", "QR-КОД ДЛЯ ОПЛАТЫ", role="section", size=9, bold=True,
    align="center")

PAY_LINES = [
    (33, "1) без комиссии — в отделениях АО «Россельхозбанк» "
         "(через операционистов);"),
    (34, "2) по тарифам банка — в отделениях ПАО Сбербанк "
         "(банкоматы, терминалы, операционисты);"),
    (35, "3) в мобильном приложении любого банка — по QR-коду "
         "(ГОСТ Р 56042-2014)."),
]
for row, text in PAY_LINES:
    ws.row_dimensions[row].height = 14
    put(ws, f"B{row}:F{row}", text, role="value", size=9, indent=1)
...
put(ws, "B38:F38", f'="Срок оплаты: до "&{ru_date(DUE_DATE)}&" г."',
    role="note", size=9, bold=True, align="center")

# Место под QR-код: макрос вставляет сюда картинку; без QR поле остаётся пустым.
put(ws, "G33:H38", "QR-код\nформируется\nмакросом", role="qr", size=9,
...
put(ws, "B40:D40", "Подпись плательщика  ______________________",
    role="plain", size=9, align="center", border=None)
put(ws, "E40:F40", "Кассир  ______________", role="plain", size=9,
    align="center", border=None)
put(ws, "G40:H40", "Дата  ____________", role="plain", size=9,
    align="center", border=None)
...
LINES = [
    ...
    ("t", "Достаточно записать лицевой счёт в ячейку КВИТАНЦИЯ!C14 (имя КВ_ЛС) — "
          "ФИО, адрес, площадь, начислено, долг, пени и итог подтянутся формулами."),
    ...
    ("t", "ИТОГО К ОПЛАТЕ берётся из столбца K листа ДАННЫЕ. Если столбец пуст, "
          "бланк считает итог сам: начислено + долг + пени + перерасчёт."),
    ("", ""),
    ("s", "4. Печать и PDF"),
    ...
    ("t", "Сама картинка QR в шаблон не встроена: вставьте изображение в область "
          "G33:H38 вручную либо подключите свой генератор в процедуре ВставитьQR "
    ...
    ("t", "• Остаток средств на специальном счёте и накопления по лицевому счёту "
          "(столбцы M и N). Пока они пусты, в разделе 4 стоит прочерк."),
...
NAMES = {
    ...
    "КВ_ПЕНИ": f"{SHEET}!$H$23",
    "КВ_ПЕРЕРАСЧЕТ": f"{SHEET}!$H$24",
    "КВ_ИТОГО": f"{SHEET}!$H$25",
    "КВ_ОПЛАЧЕНО": f"{SHEET}!$G$28",
    "КВ_НАКОПЛЕНО": f"{SHEET}!$G$29",
    "КВ_ОСТАТОК": f"{SHEET}!$G$30",
    "КВ_QR_МЕСТО": f"{SHEET}!$G$33",
    "КВ_QR": f"{CFG}!$B${QR_ROW}",
    "КВ_СХЕМА": CFG_REF["Текущая цветовая схема"],
    "ДАННЫЕ_ТАБЛИЦА": f"{DATA}!$A$1:$R$5000",
}
