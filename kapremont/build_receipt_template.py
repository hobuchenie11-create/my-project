...
DEMO = [31202, 49, "Иванов Иван Иванович",
        ...
        674.99, 19501.38, 0, 20176.37, 13441.28, None, None,
...
hint = data.cell(4, 1,
                 ...
                 "Столбцы F, J, L, M можно оставить пустыми — бланк рассчитает "
...
PERIOD_TXT = (f'IF({lookup("$N")}="",{CFG_REF["Расчётный период (текст)"]},'
              f'{d("$N")})')
PERIOD_DATE = (f'IF({lookup("$O")}="",{CFG_REF["Дата начала периода"]},'
               f'{d("$O")})')
DUE_DATE = (f'IF({lookup("$P")}="",{CFG_REF["Срок оплаты"]},{d("$P")})')
...
REF_ROWS = [
    (27, f'="Поступило оплат по лицевому счёту с начала "&YEAR({PERIOD_DATE})'
         f'&" года, руб."',
     f"={lookup('$K', empty='0')}"),
    (28, "Размер фонда капитального ремонта, накопленного по лицевому счёту, руб.",
     f'=IF({lookup("$M")}="","—",{d("$M")})'),
    (29, "Остаток средств на специальном счёте многоквартирного дома, руб.",
     f'=IF({lookup("$L")}="","—",{d("$L")})'),
]
...
put(ws, "B37:F37", f'="Срок оплаты: до "&{ru_date(DUE_DATE)}&" г."',
...
LINES = [
    ...
    ("t", "Сама картинка QR в шаблон не встроена: вставьте изображение в область "
          "G32:H37 вручную либо подключите свой генератор в процедуре ВставитьQR "
    ...
    ("t", "• Остаток средств на специальном счёте и накопления по лицевому счёту "
          "(столбцы L и M). Пока они пусты, в разделе 4 стоит прочерк."),
...
NAMES = {
    ...
    "КВ_НАКОПЛЕНО": f"{SHEET}!$G$28",
    "КВ_ОСТАТОК": f"{SHEET}!$G$29",
    "КВ_QR_МЕСТО": f"{SHEET}!$G$32",
    "КВ_QR": f"{CFG}!$B${QR_ROW}",
    "КВ_СХЕМА": CFG_REF["Текущая цветовая схема"],
    "ДАННЫЕ_ТАБЛИЦА": f"{DATA}!$A$1:$Q$5000",
}
