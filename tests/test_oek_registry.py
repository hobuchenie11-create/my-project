...
def make_xls(path, flats=("1", "2", "3"), service="Электроэнергия",
    ...
    water_rows — строки с квартирами есть и на листах воды: так приходит
    реестр, когда дом отчитывается тому же ресурснику и по горячей воде.
    """
    book = xlwt.Workbook()
    ...
    for name, service_name in [(service.upper(), service)] + [
        ...
        if service_name == service or water_rows:
            grid += _rows(list(flats))
...
def make_xlsx(path, flats=("1", "2", "3"), water_rows=False):
    book = Workbook()
    sheet = book.active
    ...
    grid = _preamble("Горячая вода") + [HEADERS]
    if water_rows:
        grid += _rows(list(flats))
    for row in grid:
        water.append(row)
