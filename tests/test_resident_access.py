...
from bot.keyboards.menu import (BTN_HELP, BTN_HISTORY, BTN_LAST, BTN_SUBMIT,
                                main_menu)
...
def test_resident_menu_has_only_four_buttons():
    assert _buttons(main_menu(is_admin=False)) == [
        BTN_SUBMIT, BTN_LAST, BTN_HISTORY, BTN_HELP]


def test_chairman_menu_adds_the_admin_button():
    buttons = _buttons(main_menu(is_admin=True))
    assert buttons[:4] == _buttons(main_menu(is_admin=False))
