...
# Без этого в показание уходили как раз цифры из скобок: «1238(13)» — 13.
# Скобку убираем только после числа: «Хвс (30)» — это само показание.
TRAILING_FRACTION_RE = re.compile(r"(?<=\d)\s*\(\s*\d+\s*\)\s*[;.,]*\s*$")
